...
def load_policy_net(
    ...
    if task_name.startswith("bsuite"):
        ...
        run_id = params["run_id"]
        path = DATA_PATH.joinpath(f"bsuite/snapshots/{bsuite_id}_{env_noise_level}/{run_id}_full")
        ...
        policy_noise_level = params["policy_noise_level"]
        run_id = params["run_id"]
        path = DATA_PATH.joinpath(f"dm_control_suite/snapshots/{dm_control_task}_{env_noise_level}/{run_id}_full")
        policy_net = tf.saved_model.load(str(path))
