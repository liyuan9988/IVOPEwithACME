...
class TerminatePredictor(snt.Module):

    def __init__(self, environment_spec):
        super(TerminatePredictor, self).__init__()
        self._net = snt.Sequential([snt.Flatten(),
                                    snt.nets.MLP([150, 100, 50, 1], activate_final=False)])
    ...
    def __call__(self, obs, action, training=False):
        ...
        pred = self._net(tf.concat([self.flat(obs), action_aug], axis=1))
...
def make_value_func_bsuite(environment_spec) -> Tuple[snt.Module, snt.Module, snt.Module]:
    value_function = ValueFunction(environment_spec)
    instrumental_feature = InstrumentalFeature(environment_spec)
    terminate_predictor = TerminatePredictor(environment_spec)
    return value_function, instrumental_feature, terminate_predictor
