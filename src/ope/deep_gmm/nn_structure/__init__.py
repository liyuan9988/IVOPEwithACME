...
def make_ope_networks(task_id: str,
    ...
    if task_id.startswith("dm_control"):
        value_func, advsarial_function = make_value_func_dm_control(
            **network_params)
    elif task_id.startswith("bsuite"):
        value_func, advsarial_function = make_value_func_bsuite(
            environment_spec, **network_params)
    else:
        raise ValueError(f"task id {task_id} not known")

    tf2_utils.create_variables(advsarial_function,
    ...
    return value_func, advsarial_function
